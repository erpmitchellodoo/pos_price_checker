...
- Install the module `pos_price_checker`. It requires `point_of_sale`, `barcodes`,
  `website` and `base`.
...
odoo --addons-path=addons,enterprise-19.0,custom --database=odoo19 \
     -i pos_price_checker --stop-after-init --test-enable
```
...
2. Verify the module installs cleanly (`-u pos_price_checker`) and the tests pass.
